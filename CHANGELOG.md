...
- `--self-test` op `check-claude-guardrails.sh` — zeven gefabriceerde settings-files (volledig, ontbrekende regel, dode `Write(...)`, geen hook, vreemde hook, kapotte JSON, ontbrekend bestand) door het échte script, met assertie op zowel exit-code als melding. De fixtures worden met `jq` afgeleid uit de canonieke regelset, zodat de test meebeweegt als die lijst verandert — een test met een eigen kopie van de regels test alleen zichzelf. Geverifieerd met een mutatie-check: het uitschakelen van `check_dead_write_rules` laat de self-test daadwerkelijk zakken (1 van 7 fout), dus de harness kan écht falen. Draait in `smoke.yml` in een eigen job `guardrails` op de runner, niet in de distro-matrix: deze self-test heeft `jq` nodig en de kale images (`almalinux:9`, `ubuntu:24.04`, `archlinux:latest`) hebben dat niet — de eerste CI-run viel daar prompt op om. Per distro `jq` installeren zou package-manager-ruis toevoegen aan een test die juist distro-onafhankelijk is. De hook-self-test blijft wél in de matrix, want die heeft geen dependencies en bewijst zo dat hij op elke distro-bash dezelfde beslissingen neemt (21/21 op alma9 in diezelfde run).
